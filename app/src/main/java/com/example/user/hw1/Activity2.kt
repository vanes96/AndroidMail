...
class Activity2 : AppCompatActivity()
{
    ...
    private val shortTimeToStart = 1000
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        if (savedInstanceState != null)
        {
            ...
            if (secondsValue == timeToStart)
                isTimerStarted = false

            if(isTimerStarted)
                startTimer(timeToStart - secondsValue)
        }
        else
        {
            timer = createTimer(shortTimeToStart)
        }
    }
    ...
    private fun createTimer(time: Int): CountDownTimer
    {
        var timer = object : CountDownTimer(time * second.toLong(), second.toLong())
        {
            ...
            {
                secondsValue = timeToStart - ticks.toInt() / second  // timeToStart - секунд всего
                secondsText?.text = getNumberName(secondsValue)
            }
